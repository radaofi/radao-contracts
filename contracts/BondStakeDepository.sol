...
contract BondStakeDepository is Ownable {
    ...
    address public immutable RA; // token given as payment for bond
    address public immutable sRA; // token given as payment for bond
    address public immutable principle; // token used to create bond
    address public immutable treasury; // mints RA when receives principle
    ...
    struct Bond {
        uint gonsPayout; // sRA gons remaining to be paid
        uint raPayout; //ra amount at the moment of bond
    ...
    constructor ( 
        ...
        require( _RA != address(0) );
        RA = _RA;
        require( _sRA != address(0) );
        sRA = _sRA;
    ...
    function deposit( 
        ...
        require( payout >= 10000000, "Bond too small" ); // must be > 0.01 RA ( underflow protection )
        ...
        /**
            ...
            approved and
            deposited into the treasury, returning (_amount - profit) RA
         */
        ...
        if ( fee != 0 ) { // fee is transferred to dao 
            IERC20( RA ).safeTransfer( DAO, fee ); 
        }
        ...
        IERC20( RA ).approve( staking, payout );
        IStaking( staking ).stake( payout, address(this) );
        IStaking( staking ).claim( address(this) );
        uint stakeGons = ISRA(sRA).gonsForBalance(payout);
        uint vesting = 
            terms.vestingTerm.mul(stakeGons).add(_bondInfo[ _depositor ].vesting.mul(_bondInfo[ _depositor ].gonsPayout))
            .div(_bondInfo[ _depositor ].gonsPayout.add( stakeGons ));
                
        // depositor info is stored
        _bondInfo[ _depositor ] = Bond({ 
            gonsPayout: _bondInfo[ _depositor ].gonsPayout.add( stakeGons ),
            raPayout: _bondInfo[ _depositor ].raPayout.add( payout ),
            vesting: vesting,
            lastBlock: block.number,
    ...
    function redeem( address _recipient, bool _stake ) external returns ( uint ) {        
        ...
        uint _amount = ISRA( sRA ).balanceForGons(info.gonsPayout);
        emit BondRedeemed( _recipient, _amount, 0 ); // emit bond data
        IERC20( sRA ).transfer( _recipient, _amount ); // pay user everything due
        return _amount;
    }
    ...
    function maxPayout() public view returns ( uint ) {
        return IERC20( RA ).totalSupply().mul( terms.maxPayout ).div( 100000 );
    }
    ...
    /**
     *  @notice return bond info with latest sRA balance calculated from gons
     *  @param _depositor address
     *  @return payout uint
     *  @return vesting uint
    ...
    function bondInfo(address _depositor) public view returns ( uint payout, uint vesting, uint lastBlock, uint pricePaid ) {
        ...
        payout = ISRA( sRA ).balanceForGons( info.gonsPayout );
    ...
    /**
     *  @notice calculate current ratio of debt to RA supply
     *  @return debtRatio_ uint
     */
    function debtRatio() public view returns ( uint debtRatio_ ) {   
        uint supply = IERC20( RA ).totalSupply();
    ...
    /**
     *  @notice calculate amount of RA available for claim by depositor
     *  @param _depositor address
     *  @return pendingPayout_ uint
     */
    function pendingPayoutFor( address _depositor ) external view returns ( uint pendingPayout_ ) {
        uint percentVested = percentVestedFor( _depositor );
        uint payout = ISRA( sRA ).balanceForGons( _bondInfo[ _depositor ].gonsPayout );
    ...
    /**
     *  @notice allow anyone to send lost tokens (excluding principle or RA) to the DAO
     *  @return bool
     */
