...
contract RaTreasury is Ownable {
    ...
    enum MANAGING { RESERVEDEPOSITOR, RESERVESPENDER, RESERVETOKEN, RESERVEMANAGER, LIQUIDITYDEPOSITOR, LIQUIDITYTOKEN, LIQUIDITYMANAGER, DEBTOR, REWARDMANAGER, SRA }

    address public immutable RA;
    ...
    address public sRA;
    uint public sRAQueue; // Delays change to sRA address
    ...
    constructor (
        ...
        isLiquidityToken[ _RA_BUSD ] = true;
        liquidityTokens.push( _RA_BUSD );

        blocksNeededForQueue = _blocksNeededForQueue;
    }

    /**
        @notice allow approved address to deposit an asset for RA
        @param _amount uint
    ...
    function deposit( uint _amount, address _token, uint _profit ) external returns ( uint send_ ) {
        ...
        // mint RA needed and store amount of rewards for distribution
        send_ = value.sub( _profit );
        IERC20Mintable( RA ).mint( msg.sender, send_ );
    ...
    /**
        @notice allow approved address to burn RA for reserves
        @param _amount uint
        @param _token address
     */
    function withdraw( uint _amount, address _token ) external {
        ...
        uint value = valueOf( _token, _amount );
        IRAERC20( RA ).burnFrom( msg.sender, value );
    ...
    function incurDebt( uint _amount, address _token ) external {
        ...
        uint maximumDebt = IERC20( sRA ).balanceOf( msg.sender ); // Can only borrow against sRA held
    ...
    /**
        @notice allow approved address to repay borrowed reserves with RA
        @param _amount uint
     */
    function repayDebtWithRA( uint _amount ) external {
        require( isDebtor[ msg.sender ], "Not approved" );

        IRAERC20( RA ).burnFrom( msg.sender, _amount );

        debtorBalance[ msg.sender ] = debtorBalance[ msg.sender ].sub( _amount );
        totalDebt = totalDebt.sub( _amount );

        emit RepayDebt( msg.sender, RA, _amount, _amount );
    }
    ...
    function mintRewards( address _recipient, uint _amount ) external {
        ...
        IERC20Mintable( RA ).mint( _recipient, _amount );
    ...
    function excessReserves() public view returns ( uint ) {
        return totalReserves.sub( IERC20( RA ).totalSupply().sub( totalDebt ) );
    }
    ...
    /**
        @notice returns RA valuation of asset
        @param _token address
        @param _amount uint
        @return value_ uint
     */
    function valueOf( address _token, uint _amount ) public view returns ( uint value_ ) {
        if ( isReserveToken[ _token ] ) {
            // convert amount to match RA decimals
            value_ = _amount.mul( 10 ** IERC20( RA ).decimals() ).div( 10 ** IERC20( _token ).decimals() );
    ...
    function queue( MANAGING _managing, address _address ) external onlyManager() returns ( bool ) {
        ...
        } else if ( _managing == MANAGING.SRA ) { // 9
            sRAQueue = block.number.add( blocksNeededForQueue );
        } else return false;
